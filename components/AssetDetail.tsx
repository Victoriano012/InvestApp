...
import { CATEGORY_LABEL, type PortfolioSummary, type Txn } from "@/lib/types";
  ...
  return (
    <div className="space-y-5">
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-xs text-muted">
              {h.asset.symbol} · {CATEGORY_LABEL[h.asset.category]} · quoted in {h.asset.currency}
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 gap-2 md:grid-cols-4 md:gap-3">
        ...
        <Tile label="Held" value={fmtNum(h.quantity)} sub={pct ? fmtPct(h.weightPct, false) + " of portfolio" : fmtEUR(h.valueEUR)} />
        <Tile label="Unrealized" value={pct ? fmtPct(h.unrealizedPct) : fmtSignedEUR(h.unrealizedEUR)} tone={h.unrealizedEUR} />
        <Tile label="Today" value={pct ? fmtPct(h.dayChangePct) : fmtSignedEUR(h.dayChangeEUR)} tone={h.dayChangeEUR ?? 0} />
      </div>
            ...
            <table className="w-full min-w-[640px] text-sm">
              <thead>
                <tr className="border-b border-line text-left text-xs text-muted">
                  ...
                  <th className="px-3 py-2 text-right font-medium">Entry ({h.asset.currency})</th>
                  ...
                  return (
                    <tr key={lot.txnId} className={`border-b border-line/60 last:border-0 ${lot.remaining === 0 ? "opacity-50" : ""}`}>
                      ...
                      <td className="tnum px-3 py-2 text-right">
                        {fmtNum(lot.remaining)}
                        {lot.remaining !== lot.quantity && <span className="text-xs text-muted"> / {fmtNum(lot.quantity)}</span>}
                      </td>
                      <td className="tnum px-3 py-2 text-right">
          ...
          <div className="rounded-lg border border-line bg-surface">
            {sells.map((s) => (
              <div key={s.id} className="flex items-center justify-between border-b border-line/60 px-3 py-2 text-sm last:border-0">
                ...
                <span className="tnum">{fmtNum(s.quantity)} × {fmtMoney(s.price, h.asset.currency)}</span>
              </div>
            ))}
